# ワードプレスプラグイン JapaneseLanguageAutoTags
Yahoo!APIを使用して文章内の単語などを自動抽出しタグ化します.  
## 使用方法
1. Yahoo!APIのYahoo!デベロッパーネットワークページよりClient IDを取得（手順は割愛）
2. ワードプレスプラグインディレクトリにFTPなどでJapaneseLanguageAutoTagsディレクトリをアップロード
3. アップロード後、プラグインを有効
4. ワードプレスのメニューからJapaneseLanguageAutoTags選択
5. JapaneseLanguageAutoTagsのページのAPIKEY入力項目にClient IDを入力
6. APIKEY入力項目後、登録ボタンから登録を行う
7. ワードプレスで記事を書き保存ボタンを押すと自動的にタグが登録されます

※ご使用の際は必ずWebサービス by Yahoo! JAPANのクレジットは入れてご使用ください、尚、自分はそれが嫌なのでMeCabを使用して動かしています

```php:JapaneseLanguageAutoTags.php
<?php
/*
